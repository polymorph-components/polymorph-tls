...
driver-ct/      target manifest (targets.toml), the recipes
                (justfile, module `conformance-ct`), the committed
                matrix (matrix.md); results/ is generated
```

`just conformance` (from the repository root) runs the standing
matrix: build, inventory check, both standing targets under the
pinned `ct-runner`, aggregate, and the committed-matrix diff.
...
only wasi and `polymorph:test/test-context` — so the generic
component-test host runner executes every target, and a target's
identity is exactly *which delivery answered the suite*.
...
jco targets (Node, browser) are pending two upstream jco defects in
the composed p3 stream/future path; see
[#28](https://github.com/polymorph-components/polymorph-tls/issues/28).
